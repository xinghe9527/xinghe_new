      ...
      if (provider == 'google_flow') {
        ...
        try {
          ...
          if (savePath != null && savePath.isNotEmpty) {
            final timestamp = DateTime.now().millisecondsSinceEpoch;
            payload['savePath'] = '$savePath/item_${item.name}_$timestamp.png';
          }
          if (hasStyleImage) {
          ...
          if (pollResult.isSuccess) {
            ...
            if (imagePath.startsWith('http')) {
              savedPath = await _downloadAndSaveImage(imagePath, 'item_${item.name}');
            } else {
              savedPath = imagePath;
            }
            if (mounted) {
          ...
          try {
            ...
            if (pollResult.isSuccess) {
              ...
              if (imagePath != null && imagePath.isNotEmpty) {
                ...
                if (imagePath.startsWith('http')) {
                  savedImagePath = await _downloadAndSaveImage(imagePath, 'item_${item.name}');
                } else {
                  savedImagePath = imagePath;
                }
                if (mounted) {
                  setState(() {
                    _items[i] = _items[i].copyWith(imageUrl: savedImagePath);
                  });
                }
                successCount++;
                print('      ✅ 成功\n');
